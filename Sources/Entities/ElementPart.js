...
ElementPart = TiledEntity.extend({
  ...
  create: function(element) {
    ...
    this.setCurrentFrameIndex(Random.sharedRandom().probably(50) ? this.m_Id : this.m_Id + this.getHorizontalFramesCount());
    this.setCenterPosition(element.convertToWorldSpace(cc.p(0, 0)).x, element.convertToWorldSpace(cc.p(0, 0)).y);
    this.setRotation(Random.sharedRandom().random(0, 720));
    this.setOpacity(255.0);

    this.m_SpeedX = Camera.sharedCamera().coord(Random.sharedRandom().random(-300.0, 300.0));
    this.m_SpeedY = Camera.sharedCamera().coord(Random.sharedRandom().random(400.0, 700.0));
    this.m_RotationSpeed = Random.sharedRandom().random(-5.0, 5.0);
