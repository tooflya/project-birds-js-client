...
Promotion = ClippedEntity.extend({
  ...
  show: function() {
    ...
    if(DataManager.sharedManager().get(false, references.time.days) == -1) {
      DataManager.sharedManager().set(true, [references.time.reward, references.time.days], [Date.now(), 0]);
